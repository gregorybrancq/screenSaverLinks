...
Create symbolic links of your favorites images to display them
via you screensaver tool.
"""
...
## Test
# imagesDir = os.path.join(homeDir, "Config/tools/screensaverLink/Test")
...
class TagC():
    ...
    def readTag(self, fileN):
        ...
        if (width < minWidth) or (height < minHeight):
            self.log.warn(HEADER, "In  readTag size of file=" + str(fileN) + " is too small (width = " + str(
    ...
    def scanTags(self):
        ...
        if os.path.isdir(imagesDir):
            for dirpath, dirnames, filenames in os.walk(imagesDir):  # @UnusedVariable
                self.log.dbg(
                    "In  scanTags dirpath=" + str(dirpath) + " dirnames=" + str(dirnames) + " filenames=" + str(
                        filenames))
            ...
            for tagN in tagsList:
                if self.tagDict.has_key(tagN):
                    ...
                    if self.tagDict[tagN]:
                        self.log.dbg(
                            "In  createLinks installSimple tagN=" + str(tagN) + "  en=" + str(self.tagDict[tagN]))
                        installSimple = True
                    else:
                        installSimple = False
                        break
